...
def write_log(log_message, dir_name):
    with open(os.path.join(app.instance_path, 'datasets', dir_name, "raas.log"), "w+") as log_file:
        log_file.write(log_message)

@celery.task(bind=True)
def start_raas(self, language, current_user_id, name, preprocess, dataverse_key='', doi='', data_folder='',
               run_instr='',
               user_pkgs='', sample_output=None, code_btw=None, prov=None, upload = False, report = False):
    ...
    try:
        ...
        write_log("Wrote Dockerfile", data_folder)
        ...
        write_log("Built Docker image", data_folder)
        self.update_state(state='PROGRESS', meta={'current': 7, 'total': 10,
                                                  'status': 'Collecting container environment information... '})
        if(report): report = language_obj.create_report(current_user_id, name, dir_name)
        ...
        write_log("Cleaned datasets", data_folder)
