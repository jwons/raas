...
def process_potential_path_string(line, potential_path, curr_wd, report_missing, script_dir, r_file):
    ...
    if maybe_import_operation(line):
        if potential_path:
            ...
            # if the path wasn't found, report file as missing
                ...
                # Even if missing change path to basename just in case
        ...
        with open(file_to_copy, 'r', errors='replace') as infile:
            for line in infile.readlines():
                ...
                if not re.match('^#', line):
                    ...
                    # This regex specifically looks for filepaths that are passed to the keyword argument 'file = '
                    # This is a STRONG indicator the string is a filepath, and the string only needs some characters
                    # and a period with some characters after it (e.g. foo.txt) to be matched
                    potential_path = re.search('\((?:.*?file\s*=\s*|\s*)[\"\']([^\"]+\.\w+)[\"\']', line)
                    # This regex looks for potential paths that have NOT been passed to a "file =" keyword.
                    # Due to this difference, the string should also contain at LEAST ONE '/' AND a file extension.
                    # (e.g. results/foo.txt). This is slightly stricter to prevent issues with non-paths being matched.
                    less_potential_path = re.search('\((?:.*?|\s*)[\"\']([^\"]+\/+[^\"]+\.\w+)[\"\']', line)
                    if potential_path:
